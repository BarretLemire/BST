class BinarySearchTreeNode:
    ...
    def deserialize(self, tree: str) -> 'BinarySearchTreeNode':
        values = tree.split(",")
        self.i = 0
        ...
        def dfs():
            if self.i >= len(values) or values[self.i] == "N":
                self.i += 1
                return None
            node = BinarySearchTreeNode(int(values[self.i]))
            self.i += 1
            node.left = dfs()
            node.right = dfs()
            return node
...
if __name__ == '__main__':
    ...
    serialized_tree = number_tree.serialize(number_tree)
    print("Serialized Tree:", serialized_tree)
    
    deserialized_tree = number_tree
    deserialized_tree.deserialize(serialized_tree)
    print("Deserialized Tree:", deserialized_tree.in_order_traversal())
    
    if number_tree.in_order_traversal() == deserialized_tree.in_order_traversal():
        print("Test Passed")
    else:
        print("Test Failed")
